...
def main():
    ...
    try:
        ...
        for size_key, data in analysis['slippage'].items():
            print(f"\n{size_key} Order:")
            print(f"  Avg Slippage:       {data['avg_bps']} bps" if data['avg_bps'] is not None else "  Avg Slippage:       N/A")
            print(f"  Taker Fee:          {data['taker_fee_bps']} bps")
            print(f"  Total Cost:         {data['total_cost_bps']} bps" if data['total_cost_bps'] is not None else "  Total Cost:         N/A")
